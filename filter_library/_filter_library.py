...
def generate_library_df(library_json, name_sep='_'):
    """
    Generate metadata dataframe for the json file
    name_sep: separator for the compound name. eg, 'Phe_CA' -> '_' is the separator
    """
    with open(library_json, 'r') as file:
        spectrum_list = []
        for line in file:
            spectrum = json.loads(line)

            # Convert peaks to mz_list and intensity_list
            mz_list, intensity_list = zip(*spectrum['peaks'])

            spectrum['mz_ls'] = list(mz_list)
            spectrum['intensity_ls'] = list(intensity_list)

            # Rename fields to match the previous structure
            spectrum['NAME'] = spectrum.pop('compound_name')
            spectrum['SMILES'] = spectrum.pop('smiles')
            spectrum['RTINSECONDS'] = spectrum.pop('rt') * 60  # Convert minutes to seconds
            spectrum['ADDUCT'] = spectrum.pop('adduct')
            spectrum['PEPMASS'] = spectrum.pop('precursor_mz')
            spectrum['SCANS'] = spectrum.pop('scan_number')
            spectrum['PRECURSOR_PURITY'] = spectrum.pop('precursor_purity')
...
def write_tsv(df, library_tsv, out_tsv):
    ...
    # FILENAME
    lib_tsv['FILENAME'] = lib_tsv['FILENAME'].apply(lambda x: x.split('.mgf')[0] + '.mzML')
...
def main(name_sep='_'):
    ...
    library_jsons = [x for x in all_files if x.endswith('.json')]

    for library_json in library_jsons:
        print(f'Processing {library_json}')

        base_name = library_json.split('.json')[0]
        ...
        # main process
        df = generate_library_df(os.path.join('input', library_json), name_sep)
        ...
        write_tsv(df, base_name + '.tsv', out_tsv)
